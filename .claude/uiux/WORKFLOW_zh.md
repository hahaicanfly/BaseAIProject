...
> **白話優先**:下面每個步驟都可以直接用你自己的話描述需求來觸發,不需要背指令語法——文件中出現的 `@agent`/斜線指令(例如 `@uiux-agent`、`/ui-ux-pro-max`)都只是**進階/可選捷徑**,不是必要條件。
...
### 執行者
你只要說「幫我畫個 [畫面] 的草圖」之類的話就好,Agent 會接手處理。(進階/可選捷徑:`@uiux-agent` 或 `/ui-ux-pro-max`)
...
### 執行者
你只要說「評審一下這個草圖」或「這個設計哪裡有問題」就好,Agent 會接手處理。(進階/可選捷徑:`@uiux-agent` 或 `/frontend-design`)
...
- 技術棧（React / SwiftUI / Compose / Vue 等）
- Design Token 來源

### 輸出
1. **完整元件程式碼**
2. **所有狀態**：Default, Hover, Focus, Pressed, Disabled, Loading
3. **a11y 支援**：aria-label / contentDescription, 對比度, 鍵盤操作
...
你只要說「幫我設計登入畫面」之類的話就好,Agent 會自動從 Phase 1 開始。
...
### Q: 評審可以自己做嗎？
**A**: 可以,但建議找人用第三方視角評審,更容易發現盲點。你只要說「用第三方角度評審一下這個設計」就好(進階/可選捷徑:`@uiux-agent`)。
