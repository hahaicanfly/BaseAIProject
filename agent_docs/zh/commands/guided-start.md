...
3. 如果這 4 項裡有 2 項以上確實缺漏,就針對缺漏的部分發問——但分批進行,每輪 1-2 個簡短問題(有 `AskUserQuestion` 就用它,沒有就用一般文字),而且絕不重問使用者在這次對話裡已經講過的事。
4. 收集的同時,也去讀 `.claude/rules/plan-first.md` 的 Exceptions 清單(讀它當下的原文)。如果這個需求已經明顯落在某條 Exception 裡(單一檔案 < 20 行、純格式調整、已經定位原因的 bug 修正、或使用者明講「直接做」)→ 直接跳過 Step 3,進入直接執行。用一句話明講適用哪一條 Exception。
