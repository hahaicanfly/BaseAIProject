給 BaseAIProject 非技術背景使用者的引導式、自然對話入口指令。把一句白話需求轉成:(0) 偵測是否在接續進行中的工作、(1) 用一兩個針對性問題補齊還缺的意圖欄位、(3) 把路由判斷交還給本 session 注入的 tier pack 裡的路由表、(4) 工作完成後,把驗收證據翻成白話。這個指令**不新增任何自己的治理判斷**——每一個路由決定、每一條驗收標準,都來自一份本來就存在的檔案,而且每次用到時都是即時重讀,不是憑記憶複製。
...
## Step 3: 交由 tier pack 的路由表決定

這是本指令唯一會做的判斷,而且嚴格來說這不是本指令自己的判斷——是 CLAUDE.md 的。

1. **現在**就去讀本 session tier pack 裡 **「Before acting」** 段落的即時原文,讀全文——`.claude/tiers/strong.md`、`mid.md` 或 `light.md`,以實際注入的那一份為準(哪一份見 `.claude/tiers/README.md`)。要真的去讀檔,不是意譯,也不是靠注入版的印象。F-003 之前這張表放在 `CLAUDE.md`「Decision Tree Before Acting」,現在已經不在那裡了。
...
- `.claude/tiers/` 下的 `strong.md` / `mid.md` / `light.md` — Step 3 交由其中「Before acting」路由表決定;拿到哪一份見 `.claude/tiers/README.md`
