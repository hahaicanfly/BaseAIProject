...
**情境**：派出的子 agent 收到 idle 通知（工具完成但未偵測到 agent 輸出）時，主對話必須區分兩種情況：
1. **尚未回報** —— agent 還在思考或有延遲，需要主動催收
2. **真正失敗** —— agent 遇到終端錯誤或已無法觸及

**協議**：

1. **收到 idle 訊號時**（來自任務系統）：
   - **不要**立刻對使用者回報失敗
   - 記下 idle 通知的時間
...
3. **主動催收**（等待期 1 後仍無輸出）：
   - 用 `SendMessage` 指定 `to: <agent_id>` 明確要求回報
   - 訊息內容：一行摘要，如「仍在等你的最終報告：[目標摘要]。請立即輸出結果。」
   - 附上期限：「再等你 10 秒回應。」

4. **等待期 2**（SendMessage 後 10–15 秒）：
   - agent 此時已收到明確通知
   - 多數 agent 會在這個窗口送出最終輸出

5. **判定**（等待期 2 後仍無輸出）：
   - 此時才可以認定真正失敗
   - **不要**對使用者說：「子 agent 沒交報告」
   - 改用 `[HUMAN_ATTENTION_REQUIRED: subagent-timeout]` 並說明：
     - 哪個子 agent、哪個任務
     - 等了多久
     - 預期的輸出是什麼
     - 建議後備方案：機械驗證、改派 agent、或升級

**反模式**：
- ❌ 「Agent A 進入 idle，所以我判定它失敗並告訴使用者」
- ✅ 「Agent A 進入 idle。我先等待、再主動催收，真正靜默無回應才回報 timeout（不是失敗）」

**與機械驗證的互補性**：
- 機械腳本（lint、測試、驗收執行）與 agent 判斷是**互補**，不可互相替代
- agent 沒回報但機械驗證通過：回報機械證據，而非「agent 失敗」
- 機械驗證失敗但 agent 說「看起來沒問題」：採用機械證據；單憑 agent 判斷不算證明
